...
function createNormal(insert) {
    ...
    self.changeScrollTarget = function (cssSelector, silent) {
        const scrollNodes = cssSelector ? document.querySelectorAll(cssSelector) : [];
        if (scrollNodes.length > 0) {
            scrollIndex = (scrollIndex + 1) % scrollNodes.length;
            var sn = scrollNodes[scrollIndex];
            self.setScrollTarget(sn, silent)
        }
    }

    self.setScrollTarget = function (element, silent) {
